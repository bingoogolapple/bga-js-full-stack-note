class PostMessageHelper {
  ...
  generateType(sign, type) {
    return `@${sign}/${this.groupName}/${type}`;
  }
  ...
  isMessageInvalid(event) {
    if (
      ...
      !(
        event.data?.type?.startsWith("@syn") ||
        event.data?.type?.startsWith("@ack")
      )
    ) {
      this.warn(
    ...
    const { error, tagName, type } = event.data;
    const logMsg = `收到 ${tagName || event.origin} 的 ${type}`;
    if (error) {
  ...
  request(type, data, timeout = 1000) {
    return new Promise((resolve, reject) => {
      const message = {
        type: this.generateType("syn", type),
      ...
      this.info(
        `向 ${this.targetTagName || this.targetOrigin} 发送 ${message.type}`,
        message
      );

      const messageHistory = {
        ...message,
        type: message.type.replace("@syn", "@ack"),
        resolve,
        reject,
      };
  ...
  on(type, listener) {
    ...
    this.listenerInfoList.push(listenerInfo);
    this.info(`注册 ${listenerInfo.type}`);
  }
  ...
  async handleReq(event) {
    ...
    for (const listenerInfo of this.listenerInfoList) {
      if (type === listenerInfo.type) {
        ...
        const logMsg = `向 ${this.targetTagName || this.targetOrigin} 发送 ${
          message.type
        }`;
        if (resultError) {
   ...
   */
  handleRes(event) {
    const { uuid, type, error } = event.data;
    if (!type.startsWith("@ack")) {
      return;
    }
