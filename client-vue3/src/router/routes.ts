...
const routes: Array<RouteRecordRaw> = [
  ...
  },
  {
    ...
    path: '/user-:pathMatch(.*)*', // 常规参数只会匹配被 / 分隔的 URL 片段中的字符。如果想匹配特定前缀的路径，可以使用通配符 :pathMatch(.*)*
    component: NotFound // 路径为 /user-admin 时，this.$route.params.pathMatch 为 admin
  },
  {
    path: '/:pathMatch(.*)*', // 常规参数只会匹配被 / 分隔的 URL 片段中的字符。如果想匹配任意路径，可以使用通配符 :pathMatch(.*)*
    name: 'not-found',
