...
import { AuthActionType, RootState } from './models'
import {
  withRouter,
  RouteComponentProps,
  Switch,
...
class AuthDva extends React.Component<RouteComponentProps> {
  render() {
...
export default withRouter(AuthDva)
...
const userListConnector = connect(
  ({ auth, loading }: RootState) => {
    return {
      ...
      loading:
        !!loading.effects[AuthActionType.listType] ||
        !!loading.effects[AuthActionType.logoutType]
    }
  },
  (dispatch: Dispatch) => {
    return {
      list() {
        dispatch(AuthActionType.listAction())
      },
      logout() {
        dispatch(AuthActionType.logoutAction())
      }
    }
  }
)
...
const loginConnector = connect(
  ({ auth, loading }: RootState) => {
    return {
      userInfo: auth.userInfo,
      loading: !!loading.effects[AuthActionType.loginType]
    }
  },
  (dispatch: Dispatch) => {
    return {
      login(username: string, password: string) {
        return dispatch(AuthActionType.loginAction({ username, password }))
      }
    }
  }
)
...
> {
  render() {
    if (this.props.userInfo) {
      return <Redirect to="/reactDemo/reduxDva/userListWithRequireAuth" />
    }
